...
describe('policyCatalog', () => {
  it('returns preview chain when preview enabled', () => {
    ...
    expect(chain[2]?.model).toBe(PREVIEW_GEMINI_FLASH_MODEL);
    expect(chain[3]?.model).toBe(DEFAULT_GEMINI_FLASH_MODEL);
    expect(chain[4]?.model).toBe(DEFAULT_GEMINI_FLASH_LITE_MODEL);
  });
